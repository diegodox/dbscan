...
impl<F: PartialOrd> DBScan<F> {
    ...
    where
        DistFn: Fn(&T, &T) -> F,
    {
        DBScanRunner::new(self, data, distance).classify()
    }
}
...
struct DBScanRunner<'p, 'd, T, F, DistFn> {
    param: &'p DBScan<F>,
    class: Vec<Class>,
    is_visited: Vec<bool>,
    data: &'d [T],
...
impl<'p, 'd, T, F, DistFn> DBScanRunner<'p, 'd, T, F, DistFn> {
    fn new(param: &'p DBScan<F>, data: &'d [T], distance: DistFn) -> Self {
        Self {
            param,
...
where
    ...
    F: PartialOrd,
{
    fn classify(mut self) -> Vec<Class> {
